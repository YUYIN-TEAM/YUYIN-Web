function postData() {
  ...
  upload_video_url = JSON.parse(upload_video_url);
  console.log(project_name, upload_video_url);
  ...
  //     upload_video_url.urls[0] +
  //     '" type="video/mp4" /></video>';
  //   var newitv = "#itv2"
  //   $(newitv).append(loaded_video);
  $.ajax({
    url: url /*接口域名地址*/,
    type: "post",
    data: formData,
    ...
    success: function (res) {
      ...
      console.log("after", upload_video_url);
      sessionStorage.setItem(
        "upload_video_url",
        JSON.stringify(upload_video_url)
      );
      reloadVideoView();
...
function delete_video(video_id){
    ...
    var upload_video_url = sessionStorage.getItem("upload_video_url");
    upload_video_url = JSON.parse(upload_video_url);
    var urls = upload_video_url.urls;
    console.log(urls[video_id])
    var file_name = urls[video_id].split('/').pop()
    console.log(file_name)
    upload_video_url.urls.splice(video_id,1)
    console.log(upload_video_url.urls)
    // $.ajax({
...
function reloadVideoView() {
  ...
  for (let i = 0; i < urls.length; i++) {
    ...
    var loaded_video =
      '<video class="itv" controls><source src="' +
      upload_video_url.urls[i] +
      '" type="video/mp4" /></video>';
    var btn = "<button class='button btn-primary' onclick = delete_video(this.id) id=" + i + "> Delete </button>";
